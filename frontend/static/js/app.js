...
function initializeSystemStats() {
    if (appState.settings.systemStats) {
        updateSystemStats();
        appState.statsInterval = setInterval(updateSystemStats, appState.settings.statsFrequency);
    }
}
...
function renderConsoleLogs() {
    ...
    if (filter !== 'all') {
        ...
        filteredLogs = appState.consoleLogs.filter(log => log.type === typeMap[filter]);
    }
    
    // Render
    container.innerHTML = filteredLogs.map(log => {
        const typeClass = log.type === 'error' ? 'console-line-error' :
                         log.type === 'warning' ? 'console-line-warning' :
                         log.type === 'success' ? 'console-line-success' :
                         'console-line-info';
...
function exportLogs() {
    const logsText = appState.consoleLogs.map(log => 
...
async function handleOCRUpload(files) {
    ...
    const progressInterval = setInterval(() => {
        ...
        if (progress <= 90) {
            ...
            document.getElementById('ocr-progress-text').textContent = `Processing... ${progress}%`;
        }
    }, 500);
    ...
    try {
        ...
        setTimeout(() => {
            ...
            logToConsole(`OCR: Completed processing ${data.results.length} file(s)`, 'success');
        }, 300);
    } catch (error) {
        clearInterval(progressInterval);
    ...
    results.forEach(result => {
        ...
        if (result.success) {
            ...
            card.innerHTML = `
                ...
                <div class="result-text">${escapeHtml(result.text) || '<em>No text detected</em>'}</div>
                <button class="btn btn-secondary copy-btn" onclick="copyToClipboard(\`${escapeHtml(result.text).replace(/`/g, "\\`")}\`)">Copy Text</button>
            `;
        } else {
            card.innerHTML = `<div class="error-message">${escapeHtml(result.filename)}: ${escapeHtml(result.error)}</div>`;
        }
...
function initializeTrainerModule() {
    ...
    // API Import
    document.getElementById('import-api-btn').addEventListener('click', handleAPIImport);
    
    // Model category selection
    document.getElementById('model-category-select').addEventListener('change', handleCategoryChange);
    
    // Model type selection
    document.getElementById('model-type-select').addEventListener('change', handleModelTypeChange);
    ...
    document.getElementById('enable-hyperparameter-tuning').addEventListener('change', function(e) {
        document.getElementById('hyperparameter-tuning-section').style.display = e.target.checked ? 'block' : 'none';
    });
    
    // Custom code toggle
    document.getElementById('enable-custom-code').addEventListener('change', function(e) {
        document.getElementById('custom-code-section').style.display = e.target.checked ? 'block' : 'none';
    });
    ...
    if (modelType === 'polynomial') {
        ...
        paramsContainer.innerHTML = `
            ...
            <div class="setting-item">
                ...
                <input type="number" id="param-max_depth" class="terminal-input" value="${modelType === 'xgboost' ? 6 : 3}" min="1" max="15">
            </div>
        `;
        ...
        `;
    } else if (modelType === 'random_forest_classifier' || modelType === 'gradient_boosting_classifier' || modelType === 'xgboost_classifier') {
        paramsSection.style.display = 'block';
        paramsContainer.innerHTML = `
            <div class="setting-item">
                <label>Number of Estimators</label>
            ...
            <div class="setting-item">
                <label>Max Depth</label>
                <input type="number" id="param-max_depth" class="terminal-input" value="${modelType === 'xgboost_classifier' ? 6 : 3}" min="1" max="15">
            </div>
        `;
...
function getModelParams(modelType) {
    ...
    if (modelType === 'polynomial') {
        params.degree = parseInt(document.getElementById('param-degree')?.value || 2);
    } else if (modelType === 'ridge' || modelType === 'lasso') {
        params.alpha = parseFloat(document.getElementById('param-alpha')?.value || 1.0);
    } else if (modelType === 'random_forest' || modelType === 'random_forest_classifier') {
        params.n_estimators = parseInt(document.getElementById('param-n_estimators')?.value || 100);
        const maxDepth = document.getElementById('param-max_depth')?.value;
        params.max_depth = maxDepth ? parseInt(maxDepth) : null;
    } else if (modelType === 'gradient_boosting' || modelType === 'xgboost' || modelType === 'gradient_boosting_classifier' || modelType === 'xgboost_classifier') {
        params.n_estimators = parseInt(document.getElementById('param-n_estimators')?.value || 100);
        params.learning_rate = parseFloat(document.getElementById('param-learning_rate')?.value || 0.1);
        params.max_depth = parseInt(document.getElementById('param-max_depth')?.value || 3);
    } else if (modelType === 'svr' || modelType === 'svc') {
        ...
        params.C = parseFloat(document.getElementById('param-C')?.value || 1.0);
        params.max_iter = parseInt(document.getElementById('param-max_iter')?.value || 1000);
    } else if (modelType === 'kmeans') {
        params.n_clusters = parseInt(document.getElementById('param-n_clusters')?.value || 3);
        params.max_iter = parseInt(document.getElementById('param-max_iter')?.value || 300);
    } else if (modelType === 'dbscan') {
        params.eps = parseFloat(document.getElementById('param-eps')?.value || 0.5);
        params.min_samples = parseInt(document.getElementById('param-min_samples')?.value || 5);
    } else if (modelType === 'hierarchical') {
        params.n_clusters = parseInt(document.getElementById('param-n_clusters')?.value || 3);
        params.linkage = document.getElementById('param-linkage')?.value || 'ward';
    } else if (modelType === 'pca') {
        params.n_components = parseInt(document.getElementById('param-n_components')?.value || 2);
    }
...
async function handleAPIImport() {
    ...
    previewContainer.innerHTML = '<div class="spinner"></div> Importing from API...';
    ...
    try {
        ...
        if (headersText.trim()) {
            try {
                ...
                previewContainer.innerHTML = '<div class="error-message">Invalid JSON in headers</div>';
        ...
        if (bodyText.trim() && method === 'POST') {
            try {
                ...
                previewContainer.innerHTML = '<div class="error-message">Invalid JSON in body</div>';
        ...
        if (data.success) {
            ...
            logToConsole(`API Import: Successfully imported ${data.row_count} rows`, 'success');
        } else {
...
async function refreshCSVPreview() {
    ...
    logToConsole(`Refreshing CSV preview: rows ${rowStart}-${rowEnd}, columns ${colStart}-${colEnd}`, 'info');
    alert(`Preview refresh requested.\nRows: ${rowStart}-${rowEnd}\nColumns: ${colStart}-${colEnd}\n\nNote: Full preview refresh requires re-uploading the file with new range parameters.`);
}
...
async function handleCSVUpload(file) {
    ...
    try {
        ...
        if (data.success) {
            ...
            logToConsole(`Model Trainer: CSV loaded successfully (${data.row_count} rows, ${data.column_count} columns)`, 'success');
        } else {
            previewContainer.innerHTML = `<div class="error-message">${data.error}</div>`;
...
function displayCSVPreview(data) {
    ...
    let tableHTML = `
        <div class="success-message">
            File uploaded: ${data.filename} (${data.row_count} rows, ${data.column_count} columns)
        </div>
...
function showDataPreparationStep(data) {
        ...
        headerHTML += `
            <div class="warning-message" style="margin-bottom: 15px;">
                ...
                ${highMissingCols.map(col => {
                    const percent = ((col.null_count / data.row_count) * 100).toFixed(1);
    ...
    data.columns.forEach(col => {
        ...
        let columnHTML = `
            <input type="checkbox" id="col-${col.name}" data-column="${col.name}" class="feature-checkbox">
        ...
        if (col.null_count > 0) {
            columnHTML += `<span class="column-warning" style="color: var(--warning); font-size: 0.85em; margin-left: 10px;">(${col.null_count} missing - ${missingPercent.toFixed(1)}%)</span>`;
        }
    ...
    data.columns.forEach(col => {
        if (col.null_count > 0) {
            ...
            item.innerHTML = `
                <span>${escapeHtml(col.name)} (${col.null_count} missing):</span>
    ...
    splitContainer.innerHTML = `
        ...
        <div class="split-display">
            <span id="split-display-text">Training: ${Math.round(data.row_count * 0.8)} rows, Testing: ${Math.round(data.row_count * 0.2)} rows</span>
        </div>
    `;
    ...
    document.getElementById('split-slider').addEventListener('input', (e) => {
        ...
        document.getElementById('split-display-text').textContent = 
            `Training: ${trainRows} rows, Testing: ${testRows} rows`;
    });
...
async function startTraining() {
    ...
    const featureColumns = Array.from(document.querySelectorAll('.feature-checkbox:checked'))
        .map(cb => cb.getAttribute('data-column'))
        .filter(col => !appState.droppedColumns || !appState.droppedColumns.has(col)); // Exclude dropped columns
    
    const targetColumn = document.querySelector('input[name="target-column"]:checked')?.value;
    
    // Ensure target column is not dropped
    if (targetColumn && appState.droppedColumns && appState.droppedColumns.has(targetColumn)) {
        alert('Target column has been dropped. Please select a different target column.');
    ...
    const modelName = document.getElementById('model-name').value || `model_${Date.now()}`;
    ...
    const enableTuning = document.getElementById('enable-hyperparameter-tuning').checked;
    ...
    const enableCustomCode = document.getElementById('enable-custom-code').checked;
    const customCode = enableCustomCode ? document.getElementById('custom-code').value : null;
    ...
    const trainingMessage = `
        <div style="background: var(--bg-surface); border: 2px solid var(--accent); border-radius: 12px; padding: 20px; text-align: center;">
            ...
            <p style="color: var(--text-secondary); margin-bottom: 4px;">${modelType.replace(/_/g, ' ').toUpperCase()}</p>
            ${enableTuning ? `<p style="color: var(--warning); font-size: 0.9em;">⚠️ Hyperparameter tuning enabled (${tuningConfig.method})</p>
            <p style="color: var(--text-secondary); font-size: 0.85em;">Estimated time: ${getTuningTimeEstimate(tuningConfig.method)}</p>` : ''}
    ...
    if (enableTuning) {
        logToConsole(`Training: Hyperparameter tuning enabled (${tuningConfig.method})`, 'info');
    }
    logToConsole(`Training: Target=${targetColumn}, Features=[${featureColumns.join(', ')}]`, 'info');
    ...
    try {
        ...
        if (data.success) {
            ...
            logToConsole(`Training: Model "${modelName}" trained successfully ${metricInfo ? `(${metricInfo})` : ''}`, 'success');
    ...
    if (data.metrics.r2_score !== undefined) {
        ...
        if (settings.r2Score !== false) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.r2_score.toFixed(4)}</span>
                </div>`;
        }
        ...
        if (settings.rmse !== false) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.rmse.toFixed(4)}</span>
                </div>`;
        }
        ...
        if (settings.mae !== false) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.mae.toFixed(4)}</span>
                </div>`;
        }
        ...
        if (settings.mse && data.metrics.mse) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.mse.toFixed(4)}</span>
                </div>`;
        }
        ...
        if (settings.adjustedR2 && data.metrics.adjusted_r2) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.adjusted_r2.toFixed(4)}</span>
                </div>`;
        }
    } else if (data.metrics.accuracy !== undefined) {
        ...
        metricsHTML += `
            <div class="metric">
                ...
                <span class="metric-value">${data.metrics.accuracy.toFixed(4)}</span>
            </div>
            <div class="metric">
                <span class="metric-label">Precision:</span>
                <span class="metric-value">${data.metrics.precision.toFixed(4)}</span>
            </div>
            <div class="metric">
                <span class="metric-label">Recall:</span>
                <span class="metric-value">${data.metrics.recall.toFixed(4)}</span>
            </div>
            <div class="metric">
                <span class="metric-label">F1 Score:</span>
                <span class="metric-value">${data.metrics.f1_score.toFixed(4)}</span>
            </div>`;
        ...
        if (data.metrics.roc_auc) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.roc_auc.toFixed(4)}</span>
                </div>`;
        }
        ...
        if (data.metrics.silhouette_score !== null) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.silhouette_score.toFixed(4)}</span>
                </div>`;
        }
        ...
        if (data.metrics.davies_bouldin_score !== null) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${data.metrics.davies_bouldin_score.toFixed(4)}</span>
                </div>`;
        }
    } else if (data.metrics.n_components !== undefined) {
        ...
        if (data.metrics.explained_variance_ratio) {
            metricsHTML += `
                <div class="metric">
                    ...
                    <span class="metric-value">${(data.metrics.explained_variance_ratio.reduce((a, b) => a + b, 0) * 100).toFixed(2)}%</span>
                </div>`;
        }
    }
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <div>
                        ...
                        <div style="margin-top: 4px; color: var(--text-secondary); font-size: 0.9em;">
                            <span>Time taken: <strong>${data.training_time}s</strong></span>
                            ${data.train_size ? ` | Training samples: <strong>${data.train_size}</strong>` : ''}
                            ${data.test_size ? ` | Test samples: <strong>${data.test_size}</strong>` : ''}
                        </div>
                    </div>
                </div>
            </div>
        ...
        coefficientsHTML = `
            <div style="margin-top: 20px;">
                ...
                <div class="result-text">
                    ${Object.entries(data.coefficients).map(([k, v]) => `${k}: ${typeof v === 'number' ? v.toFixed(4) : v}`).join('\n')}
                </div>
                ${data.intercept !== undefined && data.intercept !== null && typeof data.intercept === 'number' ? `<p style="margin-top: 10px;">Intercept: ${data.intercept.toFixed(4)}</p>` : ''}
            </div>
        `;
    }
...
function dropColumn(columnName) {
    ...
    if (appState.currentDataset) {
        appState.currentDataset.columns = appState.currentDataset.columns.filter(col => col.name !== columnName);
    }
    
    logToConsole(`Column dropped: ${columnName}`, 'info');
    alert(`Column "${columnName}" has been dropped and will not be used in training.`);
}

// Model Management Module
function initializeModelsModule() {
    document.getElementById('refresh-models-btn').addEventListener('click', loadModels);
    document.getElementById('load-model-btn').addEventListener('click', showModelLoader);
    document.getElementById('confirm-load-model-btn').addEventListener('click', loadSelectedModel);
    ...
    fetch('/api/models')
        ...
        .then(data => {
            ...
            data.models.forEach(model => {
                ...
                option.textContent = `${model.model_name} (${model.model_type}) - R²: ${model.metrics.r2_score?.toFixed(4)}`;
...
async function loadSelectedModel() {
    ...
    try {
        ...
        // Show message
        logToConsole(`Loaded model: ${modelData.model_name} (${modelData.model_type})`, 'success');

        // Show success message with loaded model info
        const message = `Model "${modelData.model_name}" loaded successfully!\n\n` +
...
async function loadModels() {
    ...
    try {
        ...
        if (data.models.length === 0) {
            container.innerHTML = '<div class="result-card">No models found. Train a model in the Model Trainer module.</div>';
            return;
        }
            ...
            card.innerHTML = `
                ...
                <div class="model-metrics">
                    ${model.metrics.r2_score !== undefined ? `
                        <div class="metric">
                            <span class="metric-label">R² Score:</span>
                            <span class="metric-value">${model.metrics.r2_score.toFixed(4)}</span>
                        </div>
                        <div class="metric">
                            <span class="metric-label">RMSE:</span>
                            <span class="metric-value">${model.metrics.rmse.toFixed(4)}</span>
                        </div>
                        <div class="metric">
                            <span class="metric-label">MAE:</span>
                            <span class="metric-value">${model.metrics.mae.toFixed(4)}</span>
                        </div>
                    ` : ''}
                    ${model.metrics.accuracy !== undefined ? `
                        <div class="metric">
                            <span class="metric-label">Accuracy:</span>
                            <span class="metric-value">${model.metrics.accuracy.toFixed(4)}</span>
                        </div>
                        <div class="metric">
                            <span class="metric-label">Precision:</span>
                            <span class="metric-value">${model.metrics.precision.toFixed(4)}</span>
                        </div>
                        <div class="metric">
                            <span class="metric-label">F1 Score:</span>
                            <span class="metric-value">${model.metrics.f1_score.toFixed(4)}</span>
                        </div>
                        ${model.metrics.roc_auc ? `
                            <div class="metric">
                                <span class="metric-label">ROC-AUC:</span>
                                <span class="metric-value">${model.metrics.roc_auc.toFixed(4)}</span>
                            </div>
                        ` : ''}
                    ` : ''}
                    ${model.metrics.n_clusters !== undefined ? `
                        <div class="metric">
                            ...
                            <span class="metric-value">${model.metrics.n_clusters}</span>
                        </div>
                        ${model.metrics.silhouette_score ? `
                            <div class="metric">
                                <span class="metric-label">Silhouette Score:</span>
                                <span class="metric-value">${model.metrics.silhouette_score.toFixed(4)}</span>
                            </div>
                        ` : ''}
                    ` : ''}
                    ${model.metrics.n_components !== undefined ? `
                        <div class="metric">
                            <span class="metric-label">Components:</span>
                            <span class="metric-value">${model.metrics.n_components}</span>
                        </div>
                        ${model.metrics.explained_variance_ratio ? `
                            <div class="metric">
                                <span class="metric-label">Variance Explained:</span>
                                <span class="metric-value">${(model.metrics.explained_variance_ratio.reduce((a, b) => a + b, 0) * 100).toFixed(2)}%</span>
                            </div>
                        ` : ''}
                    ` : ''}
                </div>
                <div class="model-actions">
                    <button class="btn btn-secondary" onclick="viewModelDetails('${model.model_id}')">View Details</button>
                    <button class="btn btn-secondary" onclick="downloadModel('${model.model_id}', '${model.model_name}')">Download Model</button>
                    <button class="btn btn-primary" onclick="exportNotebook('${model.model_id}')">Export Notebook</button>
                    <button class="btn btn-danger" onclick="deleteModel('${model.model_id}')">Delete</button>
                </div>
            `;
            container.appendChild(card);
        });
...
function initializeNotebookModule() {
    ...
    uploadZone.addEventListener('click', () => fileInput.click());
    fileInput.addEventListener('change', (e) => handleNotebookUpload(e.target.files[0]));
...
function displayNotebook(data) {
    ...
    data.cells.forEach(cell => {
        ...
        const cellType = cell.type === 'code' ? 'Code' : cell.type === 'markdown' ? 'Markdown' : 'Raw';
        ...
        cellDiv.innerHTML = `
            ...
            ${cell.outputs && cell.outputs.length > 0 ? `
                <div class="cell-output">
                    ...
                    ${cell.outputs.map(out => escapeHtml(out.data || out.text || '')).join('<br>')}
                </div>
            ` : ''}
        `;
    ...
    if (chartSearch) {
        chartSearch.addEventListener('input', (e) => {
            ...
            chartOptions.forEach(option => {
                ...
                const description = option.querySelector('.setting-description')?.textContent.toLowerCase() || '';
...
function switchSettingsTab(tabName) {
    ...
    const targetNavBtn = document.querySelector(`[data-settings-tab="${tabName}"]`);
    if (targetNavBtn) {
...
function loadSettingsIntoUI() {
    ...
    if (scanlinesCheckbox) scanlinesCheckbox.checked = appState.settings.scanlines !== false;
    if (flickerCheckbox) flickerCheckbox.checked = appState.settings.flicker !== false;
    if (glowCheckbox) glowCheckbox.checked = appState.settings.glow !== false;
    if (vignetteCheckbox) vignetteCheckbox.checked = appState.settings.vignette !== false;
    if (curvatureCheckbox) curvatureCheckbox.checked = appState.settings.crtCurvature || false;
    if (brightnessSlider) brightnessSlider.value = appState.settings.crtBrightness || 100;
    if (brightnessValue) brightnessValue.textContent = `${appState.settings.crtBrightness || 100}%`;
    
    // System stats
    const systemStatsCheckbox = document.getElementById('setting-system-stats');
    const statsFrequencySelect = document.getElementById('setting-stats-frequency');
    ...
    if (appState.settings.showMetrics) {
        document.getElementById('metric-r2').checked = appState.settings.showMetrics.r2Score !== false;
        document.getElementById('metric-rmse').checked = appState.settings.showMetrics.rmse !== false;
        document.getElementById('metric-mae').checked = appState.settings.showMetrics.mae !== false;
        document.getElementById('metric-mse').checked = appState.settings.showMetrics.mse || false;
        document.getElementById('metric-adjusted-r2').checked = appState.settings.showMetrics.adjustedR2 || false;
    }
    
    // Training charts
    if (appState.settings.showCharts) {
        const charts = appState.settings.showCharts;
        document.getElementById('chart-actual-vs-predicted').checked = charts.actualVsPredicted !== false;
        document.getElementById('chart-residuals').checked = charts.residualPlot !== false;
        document.getElementById('chart-feature-importance').checked = charts.featureImportance !== false;
        document.getElementById('chart-distribution').checked = charts.distribution || false;
        
        // New chart options
        const learningCurveEl = document.getElementById('chart-learning-curve');
        if (learningCurveEl) learningCurveEl.checked = charts.learningCurve || false;
        
        const correlationMatrixEl = document.getElementById('chart-correlation-matrix');
        if (correlationMatrixEl) correlationMatrixEl.checked = charts.correlationMatrix || false;
        
        const confusionMatrixEl = document.getElementById('chart-confusion-matrix');
        if (confusionMatrixEl) confusionMatrixEl.checked = charts.confusionMatrix !== false;
        ...
        if (clusterScatterEl) clusterScatterEl.checked = charts.clusterScatter !== false;
        
        const screePlotEl = document.getElementById('chart-scree-plot');
        if (screePlotEl) screePlotEl.checked = charts.screePlot !== false;
        
        const transformedScatterEl = document.getElementById('chart-transformed-scatter');
        if (transformedScatterEl) transformedScatterEl.checked = charts.transformedScatter !== false;
    }
}
...
function applySettings() {
    ...
    // Apply theme
    body.classList.remove('theme-amber', 'theme-blue', 'theme-white', 'theme-red', 'theme-purple', 'theme-matrix', 'theme-hacker', 'theme-light');
    if (appState.settings.theme !== 'green') {
        body.classList.add(`theme-${appState.settings.theme}`);
    }
    
    // Apply curvature
    body.classList.toggle('curvature-enabled', appState.settings.crtCurvature || false);
    
    // Apply brightness
    document.documentElement.style.setProperty('--screen-brightness', `${appState.settings.crtBrightness || 100}%`);
}
...
function saveSettings() {
    ...
    if (scanlinesCheckbox) appState.settings.scanlines = scanlinesCheckbox.checked;
    ...
    if (curvatureCheckbox) appState.settings.crtCurvature = curvatureCheckbox.checked;
    if (brightnessSlider) appState.settings.crtBrightness = parseInt(brightnessSlider.value);
    ...
    appState.settings.showCharts = {
        actualVsPredicted: document.getElementById('chart-actual-vs-predicted').checked,
        residualPlot: document.getElementById('chart-residuals').checked,
        featureImportance: document.getElementById('chart-feature-importance').checked,
        distribution: document.getElementById('chart-distribution').checked,
        learningCurve: document.getElementById('chart-learning-curve')?.checked || false,
        correlationMatrix: document.getElementById('chart-correlation-matrix')?.checked || false,
        confusionMatrix: document.getElementById('chart-confusion-matrix')?.checked || true,
        rocCurve: document.getElementById('chart-roc-curve')?.checked || true,
        clusterScatter: document.getElementById('chart-cluster-scatter')?.checked || true,
        screePlot: document.getElementById('chart-scree-plot')?.checked || true,
        transformedScatter: document.getElementById('chart-transformed-scatter')?.checked || true
    };
...
function showSettingsMessage(message, type) {
    ...
    const settingsContent = document.querySelector('#settings-module .module-content');
